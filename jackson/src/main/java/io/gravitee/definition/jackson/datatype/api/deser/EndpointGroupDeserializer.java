    ...
    public EndpointGroup deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException, JsonProcessingException {
        ...
        if (nodeEndpoints != null) {
            ...
            for (JsonNode jsonNode : nodeEndpoints) {
                ...
                JsonNode endpointTarget = jsonNode.path("target");
                if (endpointTarget.asText().startsWith("grpc")) {
                ...
                switch (type) {
                    case HTTP:
                        ...
                        break;
                    case HTTP2:
                        endpoint = jsonNode.traverse(jp.getCodec()).readValueAs(Http2Endpoint.class);
                        break;
                    case GRPC:
